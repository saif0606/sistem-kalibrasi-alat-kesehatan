...
public function unreadCount()
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($validated['message']) && !$attachmentPath && !$isSticker) {
    ...
    private function prosesBot(string $pesanUser)
    {
        ...
        try {
           ...
           if ($confidence < $THRESHOLD || !$intent) {
            ...
            // Tampilkan pesan bahwa bot sedang offline agar user tidak bingung
            ChatMessage::create([
                'user_id'     => Auth::id(),
                'admin_id'    => null,
                'sender_role' => 'bot',
                'message'     => "Maaf, sistem Asisten Bot saat ini sedang offline atau tidak dapat diakses (Ngrok 404). Pesan Anda telah kami terima dan akan dibalas langsung oleh Admin kami segera.",
                'is_read'     => true,
            ]);
        }
    }
