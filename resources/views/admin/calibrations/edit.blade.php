            ...
            <form action="{{ route('admin.calibrations.update', $calibration->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="jadwal-box" id="jadwal-box">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('lokasi_kalibrasi') is-invalid @enderror" name="lokasi_kalibrasi" id="lokasi_kalibrasi">
                            <option value="">-- Pilih Lokasi --</option>
                            <option value="Klinik / Faskes" {{ old('lokasi_kalibrasi', $calibration->lokasi_kalibrasi) == 'Klinik / Faskes' ? 'selected' : '' }}>Klinik / Faskes</option>
                            <option value="Lab UPTD" {{ old('lokasi_kalibrasi', $calibration->lokasi_kalibrasi) == 'Lab UPTD' ? 'selected' : '' }}>Lab UPTD</option>
                        </select>
       ...
       {{ old('rejection_reason', $calibration->rejection_reason ?? 'Lainnya') == 'Lainnya' ? 'checked' : '' }}>
                    ...
                    <div class="resubmit-toggle-box" id="resubmit-toggle-box">
                        <div class="form-check form-switch">
                            ...
                            name="allow_resubmit" id="allow_resubmit" value="1"
                            {{ old('allow_resubmit', $calibration->allow_resubmit) ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold" for="allow_resubmit" style="font-size:0.85rem;">
                                Izinkan pelanggan upload ulang dokumen (nomor pesanan tetap sama)
                            </label>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleReasonFields() {
        ...
        if (reason === 'Dokumen') {
            ...
            if (allowResubmitCheckbox) allowResubmitCheckbox.checked = false;
        }
    }
