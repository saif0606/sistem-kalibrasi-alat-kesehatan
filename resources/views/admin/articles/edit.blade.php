...
@section('page_subtitle', 'Perbarui konten artikel')
...
<form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" id="artForm">
    @csrf
            @method('PUT')
                ...
                <div class="art-form-section">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="artTitle"
                            ...
                            value="{{ old('title', $article->title) }}" placeholder="Masukkan judul artikel..." required>
                    ...
                    <div>
                        ...
                        <div class="cat-pills">
                            @foreach(['Instagram', 'Youtube', 'Pengumuman', 'Edukasi', 'Dokumentasi'] as $cat)
                                <button type="button"
                                    class="cat-pill {{ (old('category', $article->category) === $cat) ? 'active' : '' }}"
                                    onclick="pickCat(this, '{{ $cat }}')">{{ $cat }}</button>
                            @endforeach
                        </div>
                        <input type="hidden" name="category" id="catInput" value="{{ old('category', $article->category) }}" required>
                ...
                <div class="art-form-section">
                    ...
                    <div class="drop-zone" id="dropZone">
                        ...
                        <div id="dropContent">
                            ...
                            <div style="font-size:12px; color:#94a3b8; margin-top:4px;">atau klik untuk browse â€” PNG, JPG, WEBP Â· maks 2MB</div>
                        </div>
                    </div>
            ...
            <div class="preview-panel">
                ...
                <div class="card-mockup" style="border:1px solid #e2e8f0; border-radius:12px; overflow:hidden; background:#fff; box-shadow:0 4px 6px rgba(0,0,0,0.05);">
                    ...
                    <div style="padding:16px;">
                        <span id="previewCat" style="display:inline-block; font-size:11px; font-weight:700; color:#17a45c; background:rgba(23,164,92,0.1); padding:4px 10px; border-radius:12px; margin-bottom:10px;">{{ $article->category }}</span>
                        <div id="previewTitle" style="font-size:16px; font-weight:700; color:#1e293b; margin-bottom:8px; line-height:1.4;">{{ $article->title }}</div>
                        <div id="previewSubtitle" style="font-size:13px; color:#64748b; line-height:1.5;">{{ Str::limit(strip_tags($article->content), 60) }}</div>
