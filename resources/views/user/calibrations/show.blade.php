...
.page-bg-gradient {
    padding-top: 96px; padding-bottom: 80px; min-height: 100vh; 
    background:
...
.bukti-upload-card:hover { border-color: #17a45c; background: rgba(34,197,94,0.04); }
...
.bukti-filename {
    font-size: 0.85rem; font-weight: 700; color: #0f7a45; margin-top: 8px; display: block;
}
...
<div class="highlight-box hb-amber mt-3">
    ...
    <p class="mb-3" style="font-size:0.9rem; font-weight:800; color:#b45309;">
        ...
        ({{ $calibration->resubmit_deadline->locale('id')->diffForHumans() }})
    </p>
...
@endif
                    {{-- Pratinjau Dokumen pindah ke kiri jika status Pembayaran --}}
            ...
            <div class="col-lg-6">
                <div class="glass-card h-100 d-flex flex-column justify-content-between">
                    ...
                    {{-- File upload preview (multi-file) pindah ke sini jika bukan status Pembayaran --}}
                    @if($calibration->status !== 'Pembayaran')
                        {!! $alatPreviewHtml !!}
                    @endif
