...
.modal-bukti-dropzone:hover {
    border-color: #17a45c;
    background: rgba(34,197,94,0.04);
}
.modal-bukti-filename {
    font-size: 0.85rem;
    font-weight: 700;
    color: #0f7a45;
}
...
{{-- ====== MODAL: PENOLAKAN DOKUMEN (auto-show) ====== --}}
@php
    $rejectedDocAlerts = $calibrations->getCollection()->filter(fn($c) => $c->canResubmitDocuments());
@endphp
@if($rejectedDocAlerts->isNotEmpty())
<div class="modal fade" id="rejectedDocModal" tabindex="-1" aria-labelledby="rejectedDocModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" style="max-width:520px;">
        <div class="modal-content" style="border-radius:20px; border:none; overflow:hidden; box-shadow:0 20px 50px rgba(0,0,0,0.18);">
            <div style="background:linear-gradient(135deg,#ef4444,#b91c1c);padding:22px 26px 18px;">
                <div style="display:flex;align-items:center;gap:14px;">
                    <div style="width:46px;height:46px;border-radius:13px;background:rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="bi bi-exclamation-triangle-fill" style="font-size:1.3rem;color:#fff;"></i>
                    </div>
                    <div>
                        <h5 class="mb-0 fw-bold" id="rejectedDocModalLabel" style="color:#fff;">Pengajuan Ditolak &mdash; Dokumen</h5>
                        <div style="font-size:0.8rem;opacity:0.85;color:#fff;margin-top:2px;">Tindakan segera diperlukan</div>
                    </div>
                </div>
            </div>
            <div class="modal-body" style="padding:22px 26px;">
                <p style="font-size:0.875rem;color:#475569;line-height:1.65;margin-bottom:16px;">
                    Pengajuan berikut <strong style="color:#dc2626;">ditolak karena dokumen tidak lengkap</strong>.
                    Anda diberi kesempatan upload ulang dalam <strong style="color:#b45309;">1&times;24 jam</strong>.
                    Jika tidak diupload sebelum batas waktu, pesanan <strong style="color:#dc2626;">hangus dan harus pengajuan ulang</strong>.
                </p>
                @foreach($rejectedDocAlerts as $cal)
                <div style="background:rgba(239,68,68,0.07);border:1.5px solid rgba(239,68,68,0.22);border-radius:14px;padding:14px 16px;margin-bottom:10px;">
                    <div style="font-family:monospace;font-size:0.95rem;font-weight:800;color:#0c2438;">
                    ...
                    <div style="display:inline-flex;align-items:center;gap:6px;background:rgba(245,158,11,0.14);border:1px solid rgba(245,158,11,0.35);border-radius:999px;padding:4px 12px;font-size:0.77rem;font-weight:700;color:#b45309;margin-top:8px;">
                        <i class="bi bi-clock-fill"></i>
                        Batas: <span style="color:#dc2626;">{{ $cal->resubmit_deadline->locale('id')->isoFormat('D MMM YYYY, HH:mm') }} WIB</span>
                        &mdash; {{ $cal->resubmit_deadline->locale('id')->diffForHumans() }}
                    </div>
                    <div style="margin-top:10px;">
                        <a href="{{ route('user.calibrations.show', $cal) }}"
                           class="btn btn-danger btn-sm fw-bold w-100" style="border-radius:9px;padding:8px;">
                            <i class="bi bi-arrow-repeat me-1"></i> Upload Ulang Dokumen
                        </a>
                    </div>
                </div>
                @endforeach
                <div style="background:rgba(239,68,68,0.06);border:1px dashed rgba(239,68,68,0.28);border-radius:11px;padding:11px 14px;font-size:0.78rem;color:#7f1d1d;margin-top:4px;">
                    <i class="bi bi-info-circle-fill me-1"></i>
                    Nomor pesanan tetap berlaku jika Anda upload dokumen sebelum batas waktu.
                </div>
            </div>
            <div class="modal-footer" style="padding:14px 26px;border-top:1px solid rgba(239,68,68,0.1);">
...
document.addEventListener('DOMContentLoaded',function(){
    new bootstrap.Modal(document.getElementById('rejectedDocModal'),{backdrop:'static'}).show();
});
</script>
@endif

<div class="portal-page-wrap" style="padding-top: 96px; padding-bottom: 80px; min-height: 100vh; background:
