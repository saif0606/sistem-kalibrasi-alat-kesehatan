...
@php
    // Form & tampilan PERSIS SAMA untuk /login maupun /admin/login — hanya
    // endpoint submit yang menyesuaikan supaya URL /admin/login terpisah
    // sesuai permintaan integrasi, tanpa mengubah desain apa pun.
    $loginSubmitRoute = request()->routeIs('admin.login') ? 'admin.login.submit' : 'login.submit';
@endphp
...
@section('lockedContent')
    ...
    <form class="auth-locked-form" method="POST" action="{{ route($loginSubmitRoute) }}">
        @csrf
...
@section('mobileContent')
    ...
    <form class="auth-form" method="POST" action="{{ route($loginSubmitRoute) }}">
        @csrf
